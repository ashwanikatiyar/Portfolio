...
function Model({ isUserRotating, scrollScale }: { isUserRotating: boolean; scrollScale: number }) {
  ...
  useFrame(({ clock }) => {
    ...
    const lerpedScale = THREE.MathUtils.lerp(modelRef.current.scale.x, scrollScale, 0.1);
    modelRef.current.scale.setScalar(lerpedScale);
    if (!isUserRotating) {
      modelRef.current.rotation.x = THREE.MathUtils.lerp(modelRef.current.rotation.x, DEFAULT_ROTATION.x, 0.1);
      modelRef.current.rotation.y = THREE.MathUtils.lerp(modelRef.current.rotation.y, DEFAULT_ROTATION.y, 0.1);
      modelRef.current.rotation.z = THREE.MathUtils.lerp(modelRef.current.rotation.z, DEFAULT_ROTATION.z, 0.1);
    }
  });
      ...
      >
        ...
        >
          ...
          <Suspense
            fallback={
              <Html center>
                <div className="text-white text-9xl font-extrabold">WELCOME</div>
              </Html>
            }
          >
